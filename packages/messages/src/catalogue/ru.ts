...
export const ru: Catalogue = {
  ...
  "dns.record.resolve.unknown": {
    title: "Резолверы не сошлись в ответе по {recordType}",
    explanation:
      "Согласных между собой ответов не хватило, чтобы сделать вывод. Сразу после изменения зоны так бывает обычно: часть резолверов ещё помнит прежний ответ и будет помнить его до конца своего кэша.",
  },
  "dns.record.resolve.unknown.split": {
    title: "Резолверы не сошлись в ответе по {recordType}",
    fact: "Видят запись: {seeing}. Не видят: {missing}",
    explanation:
      "Согласных между собой ответов не хватило, чтобы сделать вывод. Сразу после изменения зоны так бывает обычно: часть резолверов ещё помнит прежний ответ и будет помнить его до конца своего кэша.",
  },
  ...
  "dns.record.consistency.fail": {
    ...
    fact: "Видят запись: {seeing}. Не видят: {missing}",
    explanation:
      "Одни резолверы видят запись, другие нет. Чаще всего так выглядит недавнее изменение зоны, которое ещё не разошлось по кэшам; реже — расхождение между авторитетными серверами.",
    impact: "Посетители могут получать разные ответы в зависимости от используемого резолвера.",
    recommendation:
      "Если запись недавно добавляли или меняли — подождите несколько минут и нажмите «Проверить заново». Если картина держится дольше, сверьте зону на всех авторитетных серверах.",
  },
  ...
  "registry.lookup.registered.record": {
    title: "Домен зарегистрирован, регистратор — {registrar}",
    fact: "Зарегистрирован {createdAt} · оплачен до {expiresAt}",
    explanation:
  ...
  "tls.connection.own_infrastructure": {
    title: "Этот домен размещён на инфраструктуре самого 2check",
    explanation:
      "2check не проверяет собственные адреса: смотреть на себя изнутри своей же сети — это не тот взгляд снаружи, который сервис обещает, и результат был бы недостоверным. Сертификат и соединение этого домена проверьте сторонним инструментом.",
  },
