...
PRD конкретизирует концепцию: описывает контракты данных, проверки DNS/регистрации/TLS, интерфейс, безопасность и 221 критериев приёмки.
...
`pnpm dev` поднимает внутренний веб-API на порту 3001 и веб-интерфейс на 3000. Откройте http://localhost:3000 и введите домен: работают все три категории MVP и русский интерфейс с вердиктом и оценкой: DNS — сравнение четырёх резолверов, существование имени и согласованность ответов; регистрация — RDAP по зоне `.uz` с резервным запросом WHOIS; SSL/TLS — соединение с закреплённым адресом и проверки сертификата.

Отдельный инструмент разработчика показывает, что готовые модули выдают для конкретного ввода:

```bash
pnpm inspect пример.uz
