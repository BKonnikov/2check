...
`pnpm dev` поднимает внутренний веб-API на порту 3001 и веб-интерфейс на 3000. Откройте http://localhost:3000 и введите домен: работают все три категории MVP: DNS — сравнение четырёх резолверов, существование имени и согласованность ответов; регистрация — RDAP по зоне `.uz` с резервным запросом WHOIS; SSL/TLS — соединение с закреплённым адресом и проверки сертификата.
